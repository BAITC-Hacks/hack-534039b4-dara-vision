...
def transfer_channel_evidence(candidates, observations, channels):
    """Pool the same migration's unsaturated channel measurements by precision.

    For multipliers <= 1 the public scoring rule is linear in the channel
    multiplier (conversion is a probability). Call-like multipliers > 1 can
    saturate and are kept as direct observations only. No effects transfer
    between different audiences or target tariffs.
    """
    by_key = {c.key: c for c in candidates}
    pools = {}
    for observation in observations:
        candidate = by_key[observation.candidate_key]
        multiplier = float(channels[candidate.channel]["conversion_multiplier"])
        if not 0 < multiplier <= 1:
            continue
        key = (candidate.cell.key, candidate.target_tariff)
        precision, weighted, prior_precision = pools.get(key, (0.0, 0.0, 0.0))
        # A prior equivalent to 80 observations on the strongest measured
        # channel preserves the original estimate when only one is measured.
        pools[key] = (precision + observation.n_actual * multiplier ** 2,
